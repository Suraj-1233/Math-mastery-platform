...
async function main() {
    ...
    for (let i = 1; i <= 10; i++) {
        ...
        const org = await prisma.organization.create({
            data: {
                name: orgName,
                contactEmail: `contact@org${i}.demo.com`,
        ...
        for (let j = 1; j <= 3; j++) {
            const teacher = await prisma.user.create({
                data: {
                    ...
                    email: `teacher${j}_org${i}@demo.com`,
                    password: passwordHash,
                    role: 'TEACHER',
                    organizationId: org.id,
                }
        ...
        for (let j = 1; j <= 2; j++) {
            const student = await prisma.user.create({
                data: {
                    ...
                    email: `student${j}_org${i}@demo.com`,
                    password: passwordHash,
                    role: 'USER',
                    organizationId: org.id,
                }
        ...
        // Create 20 Questions for this Org (shared among teachers)
        const questions: any[] = [];
        for (let k = 1; k <= 20; k++) {
            const subject = subjects[Math.floor(Math.random() * subjects.length)];
            const topic = topics[subject as keyof typeof topics][Math.floor(Math.random() * 5)];
        ...
        for (let t = 1; t <= 2; t++) {
            const testQuestions = questions.slice((t - 1) * 10, t * 10); // 10 questions each
            ...
            // Assign test to all students in the org
            for (const student of students) {
                ...
                }).catch(() => { }); // Prevent duplicates if any

                // Randomly create attempts for half the tests
                if (Math.random() > 0.5) {
